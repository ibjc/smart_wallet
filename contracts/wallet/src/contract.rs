...
use smartwallet::wallet::{
    ConfigResponse, ExecuteMsg, HotWallet, HotWalletStateResponse, InstantiateMsg, QueryMsg,
    WhitelistedContract,
};

use crate::error::ContractError;
use crate::state::{Config, HotWalletState, CONFIG, HOT_WALLETS};
use crate::tax_querier::{deduct_tax, query_balance};
...
pub fn execute(
    ...
    match msg {
        ...
        ExecuteMsg::ReplaceContractWhitelist {
            whitelisted_contracts,
        } => execute_replace_contracts(deps, info, whitelisted_contracts),
...
pub fn execute_anchor_earn_deposit(
    ...
    let hot_wallet_config = config
        .hot_wallets
        .iter()
        .find(|&x| x.address == info.sender.to_string());
    ...
    if hot_wallet_config
        .unwrap()
        .whitelisted_messages
        .iter()
        .find(|&&x| x == ANCHOR_EARN_DEPOSIT_ID)
        .is_none()
    {
        return Err(ContractError::UnauthorizedAction {});
    }

    let anchor_market_contract = config
        .whitelisted_contracts
        .iter()
        .find(|&x| x.label == String::from(ANCHOR_MARKET_CONTRACT));
    ...
    if query_balance(deps.as_ref(), info.sender.to_string(), String::from("uusd")).unwrap()
        < Uint128::from(GAS_BUFFER)
    {
    ...
    let gas_adjusted_amount = min(
        query_balance(deps.as_ref(), info.sender.to_string(), String::from("uusd")).unwrap()
            - Uint128::from(GAS_BUFFER),
        amount.into(),
    );
    ...
    Ok(Response::new()
        .add_attributes(vec![("action", "anchor_earn_deposit")])
        .add_message(earn_msg))
}
...
pub fn execute_bluna_claim_rewards(
    ...
    if hot_wallet_config
        ...
        .whitelisted_messages
        .iter()
        .find(|&&x| x == BLUNA_CLAIM_ID)
        .is_none()
    {
        return Err(ContractError::UnauthorizedAction {});
    }

    let bluna_reward_contract = config
        .whitelisted_contracts
        .iter()
        .find(|&x| x.label == String::from(BLUNA_REWARD_CONTRACT));
    ...
    Ok(Response::new()
        .add_attributes(vec![("action", "bluna_claim_rewards")])
...
pub fn execute_fill_up_gas(
    ...
    let mut hot_wallet_state = HOT_WALLETS
        ...
        .unwrap_or(HotWalletState {
            address: info.sender.to_string(),
            last_gas_fillup: env.block.time.seconds()
                - hot_wallet_config.unwrap().gas_cooldown
                - 10u64,
        });

    //cooldown check
    if hot_wallet_state.last_gas_fillup + hot_wallet_config.unwrap().gas_cooldown
    ...
    .unwrap()
        < hot_wallet_gas_need + Uint128::from(GAS_BUFFER)
    {
        return Err(ContractError::SmartWalletGas {});
    }
    ...
    Ok(Response::new()
        .add_attributes(vec![("action", "fill_up_gas")])
...
pub fn execute_command(
    ...
    Ok(Response::new()
        .add_attributes(vec![("action", "execute_command")])
        .add_message(command))
}
...
pub fn query(deps: Deps, env: Env, msg: QueryMsg) -> StdResult<Binary> {
    match msg {
        ...
        QueryMsg::HotWallet { address } => {
            Ok(to_binary(&query_hot_wallet_state(deps, env, address)?)?)
        }
    }
}
...
pub fn query_hot_wallet_state(
    deps: Deps,
    env: Env,
    address: String,
) -> StdResult<HotWalletStateResponse> {
    let hot_wallet: HotWalletState =
        HOT_WALLETS
